...
class Ueditor{
	...
	public function __construct($CONFIG=array()){
		...
		//导入设置
		$CONFIG = $CONFIG?$CONFIG:json_decode(preg_replace("/\/\*[\s\S]+?\*\//", "", file_get_contents(CONF_PATH."ueditor.json")), true);
		...
		switch($action){
		    ...
		    case 'uploadimage':
				...
				if($CONFIG['thumb']){
					$config['thumb_list'] = $CONFIG['thumb'];
				}
	...
	private function uploadFile($config,$fieldName){
		...
		if(!$info){
			...
			//缩略图
			if($config['thumb_list']){
				$thumb = array();
				$image = new \Think\Image();
				foreach($config['thumb_list'] as $key=>$value){
					$path = $info['savepath'].$value['0'].'_'.$value['1'].'/';
					if($this->mkdir($path)){
						$thumb_url = $this->rootpath.$path.$info['savename'];
						$image->open('.'.$data['url'])->thumb($value['0'],$value['1'])->save('.'.$thumb_url);
						$thumb[$value['0']] = $thumb_url;						
					}
				}				
			}
			$data['thumb'] = $thumb;
