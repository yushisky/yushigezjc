<?php
  include('../config/common.conf.php');
    if(!empty($_GET['update'])){
      $nav_id = $_GET['nav_id'];
      $sql = 'select * from mh_index_nav where nav_id = '.$nav_id;
     //p($sql);die;
      $nav_info = get_one($sql);
      // echo WEB_PATH.'/images/'.$banner_info['banner_img_url'];die;
     //p($case_info);die;
      }else{
      show_msg('非法进入','mh_index_nav_list.php');die;
      }
    if(isset($_POST['active'])&&!empty($_POST['active'])){
      // p($_POST);die;
        $nav_id = $_POST['nav_id'];
        $data['nav_title'] = $_POST['title'];
         $data['nav'] = $_POST['nav'];
        $data['nav_title2'] = $_POST['title2'];
    ...
    // p($res);die;
    if($res){
      show_msg('修改成功','mh_index_nav_list.php');
    }else{
      show_msg('修改失败');
    }
}   
	 ...
	 <div class="row">
        ...
        <form action="#" method="post" class="cmxform" enctype="multipart/form-data">
            ...
            <div class="panel-heading">
              ...
              <a href="mh_index_nav_list.php" class="btn btn-default btn-gradient dropdown-toggle"><span class="glyphicon glyphicon-chevron-left"></span></a>
            	  </div>
            </div>
            <div class="panel-body">
                ...
                <div class="form-group">
                  <div class="input-group"> <span class="input-group-addon">标题1</span>
                    <input type="text" name="title" value="<?php echo $nav_info['nav_title']?>" class="form-control">
                  </div>
                </div>
                 <div class="form-group">
                  <div class="input-group"> <span class="input-group-addon">标题2</span>
                    <input type="text" name="nav" value="<?php echo $nav_info['nav']?>" class="form-control">
                  </div>
                </div>
                <div class="form-group">
                  <div class="input-group"> <span class="input-group-addon">标题3</span>
                    <input type="text" name="title2" value="<?php echo $nav_info['nav_title2']?>" class="form-control">
                  </div>
                </div>
                <div class="form-group">
                  <div class="input-group"> <span class="input-group-addon">标题2链接</span>
                    <input type="text" name="url" value="<?php echo $nav_info['nav_url']?>" class="form-control">
                  </div>
                </div>
                 <div class="input-group">
                    <span class="input-group-addon">图片</span>
                    <input type="file" name="nav_img" id="aaa" value="<?php echo $nav_info['nav_img'];?>" class="form-control" >
                    <img width="100px" height="100" src="<?php echo WEB_PATH.'/images/'.$nav_info['nav_img'];?>">
                    <input type="hidden" name="old_nav_img" value="<?php echo $nav_info['nav_img']?>">
                </div>
                 ...
                 <div class="form-group">
                  <div class="input-group"> <span class="input-group-addon">是否显示</span>
                    <select name="is_show" class="form-control">
                          <option value="1"<?php echo 1==$nav_info['is_show']?'selected':'';?>>显示</option>
                          <option value="2"<?php echo 2==$nav_info['is_show']?'selected':'';?>>隐藏</option>
                    </select>
                  </div>
                  </div>
                  <div class="input-group"> 
                  <span class="input-group-addon">更新时间</span>
                    <input class="form-control date-picker" id="class_code_cc_time_input" type="text" data-date-format="yyyymmdd" name="time" value="<?php echo $nav_info['addtime'];?>">
                </div>
